...
// Simplified world map visualization using positioned dots
function WorldMapDots() {
  const uniqueAlerts = alerts.reduce((acc, alert) => {
    const key = `${alert.lat.toFixed(0)}-${alert.lon.toFixed(0)}`;
  ...
  return (
    <div className="relative w-full h-[300px] bg-muted/20 rounded-lg overflow-hidden border border-border/20">
      ...
      {Array.from(uniqueAlerts.values()).map((alert, i) => {
        // Convert lat/lon to x/y percentage
        const x = ((alert.lon + 180) / 360) * 100;
        const y = ((90 - alert.lat) / 180) * 100;
          ...
          >
            ...
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block z-10">
              <div className="glass-card p-2.5 rounded-lg min-w-[160px] text-[10px]">
                ...
                <p className="text-muted-foreground">Type: <span className="text-foreground">{alert.attackType}</span></p>
                <p className="text-muted-foreground">Hits: <span className="text-foreground">{alert.hitCount}</span></p>
              </div>
            </div>
          </motion.div>
        );
