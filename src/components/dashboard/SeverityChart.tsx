...
import { useSeverityDistribution } from "@/hooks/useSupabaseData";
import { motion } from "framer-motion";

export function SeverityChart() {
  const { distribution, isLoading } = useSeverityDistribution();
  const total = distribution.reduce((sum, s) => sum + s.value, 0);
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        <div className="h-[180px] flex items-center justify-center text-muted-foreground text-xs animate-pulse">Loading...</div>
      ) : (
